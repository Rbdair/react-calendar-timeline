...
ReactCalendarTimeline.defaultProps = {
  ...
  moveResizeValidator: function moveResizeValidator(action, item, time, edge, newGroupIndex, newGroup, originalGroupIndex, originalGroup) {
    return {
      time: item,
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.selectItem = function (itemId, clickType, e, item) {
    if (_this4.state.selectedItem === itemId || _this4.props.itemTouchSendsClick && clickType === 'touch') {
      if (itemId && _this4.props.onItemClick) {
        var time = _this4.timeFromItemEvent(e);
        _this4.props.onItemClick(itemId, e, time, item);
      }
    } else {
      _this4.setState({ selectedItem: itemId });
      if (itemId && _this4.props.onItemSelect) {
        var _time = _this4.timeFromItemEvent(e);
        _this4.props.onItemSelect(itemId, e, _time, item);
  ...
  this.doubleClickItem = function (itemId, e, item) {
    if (_this4.props.onItemDoubleClick) {
      var time = _this4.timeFromItemEvent(e);
      _this4.props.onItemDoubleClick(itemId, e, time, item);
    }
  };

  this.contextMenuClickItem = function (itemId, e, item) {
    if (_this4.props.onItemContextMenu) {
      var time = _this4.timeFromItemEvent(e);
      _this4.props.onItemContextMenu(itemId, e, time, item);
    }
  };
  ...
  this.dropItem = function (itemId, dragTime, newGroupOrder, previousGroupOrder, item) {
    _this4.setState({ draggingItem: null, dragTime: null, dragGroupTitle: null });
    if (_this4.props.onItemMove) {
      var group = _this4.props.groups[newGroupOrder];
      var previousGroup = _this4.props.groups[previousGroupOrder];
      _this4.props.onItemMove(itemId, dragTime, newGroupOrder, previousGroupOrder, item, group, previousGroup);
    }
  };

  this.resizingItem = function (itemId, resizeTime, edge) {
    _this4.setState({
      resizingItem: itemId,
      resizingEdge: edge,
  ...
  this.resizedItem = function (itemId, resizeTime, edge, timeDelta, item) {
    _this4.setState({ resizingItem: null, resizingEdge: null, resizeTime: null });
    if (_this4.props.onItemResize && timeDelta !== 0) {
      _this4.props.onItemResize(itemId, resizeTime, edge, item);
    }
  };

  this.onMoveResizeValidator = function (action, item, time, edge, newGroupIndex, originalGroupIndex) {
    var newGroup = _this4.props.groups[newGroupIndex];
    var originalGroup = _this4.props.groups[originalGroupIndex];
    return _this4.props.moveResizeValidator(action, item, time, edge, newGroupIndex, newGroup, originalGroupIndex, originalGroup);
  };

  this.handleRowClick = function (e, rowIndex, group) {
    ...
    var groupId = (0, _generic._get)(group, _this4.props.keys.groupIdKey);
    _this4.props.onCanvasClick(groupId, time, e, group);
  };

  this.handleRowDoubleClick = function (e, rowIndex, group) {
    if (_this4.props.onCanvasDoubleClick == null) return;

    var time = _this4.getTimeFromRowClickEvent(e);
    var groupId = (0, _generic._get)(group, _this4.props.keys.groupIdKey);
    _this4.props.onCanvasDoubleClick(groupId, time, e, group);
  };

  this.handleScrollContextMenu = function (e, rowIndex, group) {
    ...
    if (_this4.props.onCanvasContextMenu) {
      e.preventDefault();
      _this4.props.onCanvasContextMenu(groupId, timePosition, e, group);
    }
  };
  ...
  this.renderHeaders = function () {
    ...
    return _react2.default.createElement(
      ...
      _this4.props.rightSidebarWidth && _react2.default.createElement(_SidebarHeader2.default, { variant: 'right' })
    );
  };
