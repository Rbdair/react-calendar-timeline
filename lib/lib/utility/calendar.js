...
function getGroupedItems(items, groupOrders) {
  ...
  for (var i = 0; i < Object.keys(groupOrders).length; i++) {
    var groupOrder = groupOrders[Object.keys(groupOrders)[i]];
    groupedItems[i] = {
  ...
  for (var _i = 0; _i < items.length; _i++) {
    if (items[_i].dimensions.order !== undefined) {
      var _groupOrder = groupedItems[items[_i].dimensions.order.index];
      if (_groupOrder) {
        _groupOrder.items.push(items[_i]);
      }
    }
  }
...
function stackItems(items, groups, canvasTimeStart, visibleTimeStart, visibleTimeEnd, width, props, state) {
  ...
  var _stackAll = stackAll(dimensionItems, groupOrders, lineHeight, groups, stackItems),
